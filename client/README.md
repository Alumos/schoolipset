...
从 v0.2.2 起，启动 EXE 即请求管理员权限，主进程直接执行网络修改；已经提升权限或关闭 UAC 时可能不再弹窗。标准用户需要管理员凭据，使用另一管理员账户启动时设备状态属于该账户，可能需要重新绑定姓名。客户端会采集真实网卡 MAC 地址，保存当前配置，修改后在界面逐步显示 IP、子网掩码、网关、DNS、HTTPS 和 `ping baidu.com` 验证过程；验证失败尝试恢复旧配置。失败会弹窗显示具体原因，成功后立即重新心跳刷新后台状态。发布流水线提取 EXE 内嵌清单确认 requireAdministrator；这不替代真实 Win7 SP1/10/11 网卡验收。
